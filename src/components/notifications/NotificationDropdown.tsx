...
const NotificationDropdown: React.FC = () => {
  ...
  const { user, role, hasPermission } = useSupabaseAuth();
  const navigate = useNavigate();
  const [notifications, setNotifications] = useState<Notification[]>([]);
  const [loading, setLoading] = useState(true);
  const [open, setOpen] = useState(false);

  // IMPORTANT: `hasPermission` may not be referentially stable (it can be recreated on every render).
  // Never place it directly in a useEffect dependency array, otherwise we risk infinite rerenders.
  const canViewAlerts = useMemo(() => {
    return !!user && hasPermission('alerts.view');
  }, [user?.id, role]);

  // Permission-gated: don't render anything if not allowed.
  ...
  return (
    // Safety: even if mounted elsewhere, keep it admin-only.
    role !== 'admin' ? null :
